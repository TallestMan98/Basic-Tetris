...
const theTetrominoes = [lTetromino, sTetromino, tTetromino, oTetromino, iTetromino, zTetronimo]
...
//rotate the tetromino - bug when at far right or left then rotate
function rotate () {
    undraw()
    currentRotation ++
...
//the tetrominoes without rotations up next
const upNextTetrominoes = [
    [1, displayWidth+1, displayWidth*2+1, 2], //lTetromino
    [displayWidth, displayWidth+1, 1, 2], //sTetromino
...
function displayShape () {
    //remove the tetromino from the mini-grid
...
function gameOver () {
    if(current.some(index => squares[currentPosition + index].classList.contains('taken'))) {
        setInterval(alert("Game Over! Refresh page to try again..."), 1000)
        clearInterval(timerId)
    }
}
